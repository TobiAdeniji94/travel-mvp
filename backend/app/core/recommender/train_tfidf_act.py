#!/usr/bin/env python3
"""
TF-IDF Training Script for Activity Recommendations
...
class TrainingConfig:
    ...
    max_features: int = 5000  # Activities have more diverse descriptions
...
class ActivityCorpusBuilder:
    """Builds activity corpus with enhanced features"""
    
    def __init__(self, config: TrainingConfig):
        self.config = config
        self.preprocessor = TextPreprocessor(config)
    
    async def fetch_activities(self) -> Tuple[List[str], List[str]]:
        """
        Enhanced activity data fetching with error handling and validation
        ...
        try:
            async with performance_timer("activity_data_fetch"):
                async with async_session() as session:
                    ...
                    logger.info(f"Fetched {len(ids)} valid activity documents")
                    logger.info(f"Preprocessing stats: {self.preprocessor.get_stats()}")
                    
        except SQLAlchemyError as e:
            logger.error(f"Database error during activity fetch: {e}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error during activity fetch: {e}")
            raise
...
class TFIDFTrainer:
    ...
    def __init__(self, config: TrainingConfig, output_dir: Path):
        ...
        # Output file paths
        self.vec_pkl = output_dir / "tfidf_vectorizer_act.pkl"
        self.mat_npz = output_dir / "tfidf_matrix_act.npz"
        self.idmap_pkl = output_dir / "item_index_map_act.pkl"
        self.metadata_pkl = output_dir / "training_metadata_act.pkl"
...
async def main():
    ...
    try:
        ...
        # Build corpus
        logger.info("⏳ Fetching activity corpus...")
        corpus_builder = ActivityCorpusBuilder(config)
        ids, corpus = await corpus_builder.fetch_activities()
        
        if not corpus:
            logger.error("⚠️  No activity documents found! (Nothing to vectorize.)")
            return False
        ...
        logger.info("✅ Activity TF-IDF training completed successfully")
        return True
