#!/usr/bin/env python3
"""
TF-IDF Training Script for Accommodation Recommendations
...
class TrainingConfig:
    ...
    lowercase: bool = True
    strip_accents: str = "unicode"  # Normalize accents
...
class AccommodationCorpusBuilder:
    """Builds accommodation corpus with enhanced features"""
    
    def __init__(self, config: TrainingConfig):
        self.config = config
        self.preprocessor = TextPreprocessor(config)
    
    async def fetch_accommodations(self) -> Tuple[List[str], List[str]]:
        """
        Enhanced accommodation data fetching with error handling and validation
        Returns parallel lists of (ids, cleaned_texts).
        """
        ids, texts = [], []
        
        try:
            async with performance_timer("accommodation_data_fetch"):
                async with get_session() as session:
                    ...
                    for row in result.all():
                        _id, name, desc, amenities, acc_type = row
                    ...
                    logger.info(f"Fetched {len(ids)} valid accommodation documents")
                    logger.info(f"Preprocessing stats: {self.preprocessor.get_stats()}")
                    
        except SQLAlchemyError as e:
            logger.error(f"Database error during accommodation fetch: {e}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error during accommodation fetch: {e}")
            raise
...
class TFIDFTrainer:
    ...
    def __init__(self, config: TrainingConfig, output_dir: Path):
        ...
        # Output file paths
        self.vec_pkl = output_dir / "tfidf_vectorizer_acc.pkl"
        self.mat_npz = output_dir / "tfidf_matrix_acc.npz"
        self.idmap_pkl = output_dir / "item_index_map_acc.pkl"
        self.metadata_pkl = output_dir / "training_metadata_acc.pkl"
...
async def main():
    ...
    logger.info("🚀 Starting accommodation TF-IDF training")
    ...
    try:
        ...
        # Build corpus
        logger.info("⏳ Fetching accommodation corpus...")
        corpus_builder = AccommodationCorpusBuilder(config)
        ids, corpus = await corpus_builder.fetch_accommodations()
        
        if not corpus:
            logger.error("⚠️  No accommodation documents found! (Nothing to vectorize.)")
            return False
        ...
        logger.info("✅ Accommodation TF-IDF training completed successfully")
        return True
